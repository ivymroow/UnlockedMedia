...
async function api(m,t,p,n){
  if(state.mode==='backend'){
    ...
    if(token&&!n)o.headers.Authorization='Bearer '+token
    if(p)o.body=JSON.stringify(p)
    const r=await fetch(`${b}${t}`,{...o,method:m})
    if(!r.ok){if(r.status===401&&refreshToken){const f=await tryRefreshSession();if(f)return api(m,t,p,n)}const e=await r.json().catch(()=>({error:`HTTP ${r.status}`}));throw new Error(e.error)}
    return r.json()
  }
  return standalone(t)
}
...
function navigate(v,d){
  ...
  if(v==='search')h='#q='+encodeURIComponent(state.query||'')
    ...
    h='#id='+(d?.id||'')+(d?.type==='tv'?'&type=tv':'')+(d?.title?'&t='+encodeURIComponent(d.title):'')+(d?.year?'&y='+d.year:'')+ep
...
function W(){return'<div class="welcome"><div class="welcome-card"><h1>web-streaming <span class="beta-tag">beta</span></h1><p>I am currently a solo dev with no money or servers or anything. I want to make a simple streaming site that:</p><ul class="welcome-list"><li>doesn\'t spam ads</li><li>doesn\'t break half the time</li><li>doesn\'t steal your info lmao</li><li>has original UI and functionality</li></ul><p>Currently this depends on torrents and peers. I want to do direct streaming but I\'m seriously broke and API keys cost monies man.</p><p style="font-size:13px"><a href="https://github.com/ivymroow/webstreaming/blob/main/notice.md" target="_blank" style="color:var(--primary)">View project notice &amp; source</a></p><button class="btn btn-primary" style="margin-top:20px;font-size:16px;padding:14px 48px" onclick="enterSite()">Enter</button></div></div>'}
...
async function loadEpisodeSources(id,season,episode){
  ...
  state._sources=srcs
  list.innerHTML=srcs.map(src=>{const isEmbed=!!src.embedUrl;const eu=isEmbed?',\''+esc(src.embedUrl)+'\'':'';const dead=!isEmbed&&(src.seeds||0)===0&&(src.peers||0)===0;return'<div class="source-item'+(dead?' dead-source':'')+'"><div class="source-info"><span class="source-quality">'+src.quality+'</span>'+(!isEmbed?'<span class="source-size">'+fmt(src.size)+'</span><span class="source-seeds">⬆ '+(src.seeds||0)+'</span>':'')+'<span style="color:var(--text-muted);font-size:11px">'+(src.provider||'')+'</span></div><button class="source-play" style="'+(isEmbed?'background:#4ade80;color:#000':'')+'" onclick="playSource(\''+src.hash+'\','+(src.fileIndex||0)+',\''+esc(title)+' '+q+'\''+eu+')">'+(isEmbed?'▶ Play':'▶')+'</button></div>'}).join('')
}
...
function RD(d,srces,episodes){
  ...
  if(isTv){
    ...
    if(list)list.innerHTML=srces.map(s=>{const isEmbed=!!s.embedUrl;const eu=isEmbed?',\''+esc(s.embedUrl)+'\'':'';const dead=!isEmbed&&(s.seeds||0)===0&&(s.peers||0)===0;return'<div class="source-item'+(dead?' dead-source':'')+'"><div class="source-info"><span class="source-quality">'+s.quality+'</span>'+(!isEmbed?'<span class="source-size">'+fmt(s.size)+'</span><span class="source-seeds">⬆ '+(s.seeds||0)+'</span>':'')+'<span style="color:var(--text-muted);font-size:11px">'+(s.provider||'')+'</span></div><button class="source-play" style="'+(isEmbed?'background:#4ade80;color:#000':'')+'" onclick="playSource(\''+s.hash+'\','+(s.fileIndex||0)+',\''+esc(t)+'\''+eu+')">'+(isEmbed?'▶ Play':'▶')+'</button></div>'}).join('')
  }
...
function updateHashForEpisode(){if(!state.data)return;history.replaceState(null,'','#id='+state.data.id+'&type=tv&t='+encodeURIComponent(state.data._title||'')+'&y='+(state.data._year||'')+'&s='+selectedSeason+'&e='+selectedEpisode)}
...
function initPlayer(video,baseUrl,infoHash){
  ...
  video.ontimeupdate=()=>{if(!video.duration)return;const pct=(video.currentTime/video.duration)*100;fill.style.width=pct+'%';thumb.style.display='';thumb.style.left=pct+'%';timeD.textContent=fmtTime(video.currentTime)+' / '+fmtTime(video.duration);if(timeR)timeR.textContent='-'+(video.duration-video.currentTime>0?fmtTime(video.duration-video.currentTime):'0:00');if(token&&state.data?.id){const se=state.data.type==='tv'?selectedSeason:0,ep=state.data.type==='tv'?selectedEpisode:0;clearTimeout(window._st);window._st=setTimeout(async()=>{try{await fetch(baseUrl+'/api/progress/save',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({id:state.data.id,title:state._title||'',poster:state._poster||'',type:state.data.type||'movie',season:se,episode:ep,duration:Math.round(video.duration),watched:Math.round(video.currentTime),status:video.currentTime/video.duration>.9?'watched':'watching'})})}catch{}},5000)}}
...
function cp(){if(state.player)state.player=null;if(state.prevState){state.view=state.prevState.view;state.data=state.prevState.data;state.prevState=null;qs('#app').innerHTML='<header id="mainHeader"></header><main id="main"><div class="loading-screen" id="loadingScreen"><div class="spinner"></div><p>Loading...</p></div></main>';render()}else location.reload()}
...
function toggleAuthMode(){authMode=authMode==='signin'?'signup':'signin';qs('#authModalTitle').textContent=authMode==='signin'?'Sign In':'Sign Up';qs('#authToggle').innerHTML=authMode==='signin'?'Don\'t have an account? <a href="#" onclick="toggleAuthMode();return false" style="color:var(--primary)">Sign Up</a>':'Already have an account? <a href="#" onclick="toggleAuthMode();return false" style="color:var(--primary)">Sign In</a>';qs('#emailField').style.display=authMode==='signup'?'block':'none';qs('#authError').textContent=''}
