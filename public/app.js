...
async function play(hash,fi,title,quality,seeds){
  ...
  if(state.mode==='backend'){
    ...
    // Buffer full episode first, then play with seeking
    ...
    ov.innerHTML='<div class="spinner"></div><p id="dlText">Downloading episode...</p><div style="width:60%;max-width:400px;height:6px;background:var(--surface3);border-radius:3px;overflow:hidden"><div id="dlBar" style="height:100%;width:0%;background:var(--primary);border-radius:3px;transition:width .3s"></div></div><p id="dlInfo" style="font-size:13px;color:var(--text3)"></p>';
