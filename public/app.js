...
async function play(hash,fi,title,quality,seeds){
  ...
  state.view='player'
  state._dlId = null;
  qs('#app').innerHTML='<div class="player-container"><button class="player-back" onclick="cp()"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 18-6-6 6-6"/></svg> Back</button><div class="player-wrapper" id="pw"><div id="embedLoad" style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:12px;padding:40px"><div class="spinner"></div><p>Loading stream...</p></div><iframe id="embedPlayer" style="display:none;width:100%;height:100%;border:none" allowfullscreen allow="autoplay;encrypted-media"></iframe></div></div>'

  // No peers needed — embed from vidsrc.to (instant play, seeking, audio)
  const embedUrl=(state.data?.type==='tv')
    ? `https://vidsrc.to/embed/tv/${state.data?.id}/${selectedSeason||1}/${selectedEpisode||1}`
    : `https://vidsrc.to/embed/movie/${state.data?.id}`;
