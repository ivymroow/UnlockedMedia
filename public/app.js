...
function mag(h,n){const tr=[...WT,'udp://tracker.opentrackr.org:1337/announce'].map(t=>`tr=${encodeURIComponent(t)}`).join('&');return `magnet:?xt=urn:btih:${h}&dn=${encodeURIComponent(n)}&${tr}`}
...
async function play(hash,fi,title,quality,seeds){
  ...
  if(state.mode==='backend'){
    ...
    video.onerror=async function(){try{const r=await fetch(`${base}/api/stream/check/${hash}`);const e=await r.json();perr(`Stream failed. ${e.peers!==undefined?e.peers+' peers connected':'No peers'}. Try a different source.`);}catch{perr('Stream failed. Try a different source.');}}
...
function initCustomPlayer(video, baseUrl){
  ...
  let seekEnabled = false; // seeking enabled only after full download
  video.oncanplay = () => { unlockAudio(); video.play().catch(() => {}); };
