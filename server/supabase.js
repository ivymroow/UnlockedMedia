...
async function signUp(username, password, email) {
  ...
  if (!data.session) throw new Error('Check your email to confirm signup');
...
async function getProgress(userId, itemId, season, episode) {
  let q = sb.from('watch_progress').select('*').eq('user_id', userId).eq('item_id', itemId);
...
async function listProgress(userId, status, limit = 20) {
  let q = sb.from('watch_progress').select('*').eq('user_id', userId);
...
async function addToWatchlist(userId, item) {
  ...
  const { error } = await sb.from('watchlist').upsert({ user_id: userId, item_id: id, title, poster, type }, { onConflict: 'user_id,item_id' });
  if (error) throw new Error(error.message);
}
...
async function getWatchlist(userId, limit = 30) {
  const { data } = await sb.from('watchlist').select('*').eq('user_id', userId).order('added_at', { ascending: false }).limit(limit);
  return data || [];
}

async function isInWatchlist(userId, itemId) {
  const { data } = await sb.from('watchlist').select('id').eq('user_id', userId).eq('item_id', itemId).maybeSingle();
  return !!data;
}
